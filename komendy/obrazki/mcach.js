...
class mcach {
  constructor(){
    this.name = "osiagniecie",
    this.alias = ["minecraftosiagniecie"]
  }
  async run (client, msg, args) {
	...
	const background = await Canvas.loadImage('./img/mcadv.png');
 ...
 // widze ze tutaj sie dzieje ostre kopiowańsko xd xD
	...
	ctx.fillStyle = '#ffffff';
	ctx.fillText('Osiągnięcie zdobyte!', canvas.width / 4.5, 75);

	// Add an exclamation point here and below
	ctx.font = applyText(canvas, `${join}!`);
	ctx.fillStyle = '#ffffff';
	ctx.fillText(`${join}`, canvas.width / 4.25, canvas.height / 1.5);
    ...
    console.log("minkruft osiogniecie");
  }
}
module.exports = mcach;
