...
class wyscigpociagowhard {
  constructor(){
    this.name = "wyscigpociagow-hard"
    this.alias = ["wyscig-hard"]
  }
  async run (client, msg, args) {
    ...
    msg.channel.send(msg.author.tag + " rozpoczyna grę! Czekamy na minimum 2 graczy, kto chce może dołączyć komendą `*wyscigi-hard-dolacz`.")
    let collector = msg.channel.createMessageCollector(m => m)
    collector.on('collect', m => {
        if(m.content == "*wyscigi-hard-dolacz") {
            if(otwarte == false) {
                ...
                if(gracze.includes(m.author.tag) == true) {
                    ...
                    if(gracze.length >= 2) {
                        msg.channel.send("Mamy "+gracze.length+" graczy i możemy zaczynać grę! "+msg.author.tag+" może w każdej chwili wpisać `*wyscigi-hard-start` żeby zacząć wyścig.")
                    }

                }
            }
        }
        if(m.content == "*wyscigi-hard-start") {
...
module.exports = wyscigpociagowhard;
